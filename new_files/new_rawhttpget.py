...
class RawSocket:
    def __init__(self):
        ...
        self.host_url = new_utils.get_destination_url(self.arg_url)[1]
        ...
        self.FLAG_SYN = new_utils.concat_tcp_flags(new_utils.set_syn_bit(tcp.FLAGS))
        self.FLAG_ACK = new_utils.concat_tcp_flags(new_utils.set_ack_bit(tcp.FLAGS))
        self.FLAG_FIN = new_utils.concat_tcp_flags(new_utils.set_fin_bits(tcp.FLAGS))
        self.FLAG_FIN_ACK = new_utils.concat_tcp_flags(new_utils.set_fin_ack_bits(tcp.FLAGS))
        
        ''' Instantiate the IP Header Source and Destination addresses '''
        ip.SRC_ADDRESS = socket.inet_aton(new_utils.get_localhost_addr()[0])
        ...
        try:
            ...
            src_addr = new_utils.get_localhost_addr()
            src_port = new_utils.get_localhost_port(self.receiver_socket, src_addr)
    ...
    def send_packet(self, flags: int, payload: bytes):
        ...
        self.CWND = new_utils.set_congestion_control(self.CWND, tcp.ADV_WINDOW)
        ...
        while True:
            ...
            try:
                ...
                if (self.RETRANSMIT_CTR < self.RETRANSMIT_LMT):
                    ...
                    self.CWND = new_utils.set_congestion_control(self.CWND, tcp.ADV_WINDOW, True)
    ...
    def init_tcp_handshake(self):
        ...
        if (tcp_headers["seq_num"] == tcp_headers["ack_num"] - 1):
            ...
            FLAG_ACK = new_utils.concat_tcp_flags(new_utils.set_ack_bit(tcp.FLAGS))
    ...
    def run(self):
        ...
        url, host_url = new_utils.get_destination_url(arg_url)
        request_payload = new_utils.build_GET_request(url, host_url).encode(self.FORMAT)
        ...
        raw_headers, raw_body = new_utils.parse_response(appl_layer_packet.decode(self.FORMAT))
        headers = new_utils.parse_headers(raw_headers)

        # Write content to file
        filename = new_utils.get_filename(arg_url)
        new_utils.write_to_file(filename, raw_body)
